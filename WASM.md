...
| feature | 툴체인 | SIMD 경로 |
|---------|--------|-----------|
| `nightly-portable-simd` | nightly | `core::simd` Level B (8-lane) — leaf ypsi ~3.9×, yhash ~2.2× |
| `stable-portable-simd` | **stable** | `wide` crate Level B (u32x8 / u64x4) — leaf ypsi ~2.6×, yhash ~1.8× |
| (없음) | any | scalar |

> SIMD는 *단일 순열*이 아니라 leaf의 *독립 블록 8개*를 lane에 실어 가속한다
> (Level B). u64(yhash) stable은 `wide`가 최대 u64x4라 8블록을 4-lane 2 chunk로
> 처리한다. WASM `simd128`에서도 `wide`가 동작하므로 **stable 툴체인 + wasm SIMD**
> 조합이 가능하다.

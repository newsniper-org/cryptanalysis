...
> *경고: 본 문서는 **사양 초안**이다.*
> **동결 정정 (R4)**: 초안은 512-bit 설계를 기술했으나, 구현은 YSC4-p를 그대로
> 재사용해 **1024-bit**(블록 128 byte)다. 권위 파라미터는 저장소 루트
> `FROZEN-PARAMS.md` (= 코드 = Isabelle). 아래 수치는 그에 맞춰 정정됨.
...
| 매개변수 | 값 | 비고 |
|----------|----|----|
| 기반 순열 P | YSC4-p (= 본 사양의 P_y) — *그대로 재사용* | 라운드 수만 다름 |
| 상태 폭 b | **1024 비트 (16 × u64)** | YSC4-p와 동일 폭(재사용). 블록 = 128 byte |
| chaining value 크기 n | 256 비트 | digest 절단 길이; 보안 강도 결정 |
| 라운드 R_b (compress, leaf·internal) | 8 | YSC4 R=16의 절반 |
| 라운드 R_c (finalize) | 12 | 종결 라운드 |
| 라운드 R_mask (mask derive) | **24** | YSC4와 동일 (1회성, `MASK_DERIVE`) |
...
메시지 길이 ≤ `T_max × block_size` (= 8 × 128 = 1024 byte) 인 입력은:
- 트리 없이 *단일 leaf* 처리.
- 출력 = `trunc(P(Acc ⊕ maskMid(LEAF, 0)))`.
- HashMap 키의 대부분(< 1024 byte) 이 경로로 흐름.
...
- `P_y`는 YSC4-p의 24-라운드 (mask derivation은 일회성; `MASK_DERIVE = 24`).
...
위치 `pos`, 블록 `x_0, …, x_{t-1}` (t ≤ T_max, 각 블록 **128 바이트 = 1024비트**):
...
```
YHash(msg, len) :
    blocks = split(msg, block_size=128)
...
- Single-leaf fast path: 완전히 stack-only (≤ 1024 byte 키).

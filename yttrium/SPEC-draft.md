...
6. yhash 대응(u64) yttrium-large: 순열 코어·**전체 Farfalle-tree 모드 구현 완료**(scalar, 가변출력;
   `large.rs`)·구조검증(§1.2). 라운드수 *값*은 변형패밀리로 결정(동일 튜플 적용). 잔여:
   **u64 보안레벨 정당화** — 현 변형은 u32 128-bit-digest 기준이라 large의 256-bit digest에서
   full 2¹²⁸ acc-저항엔 R_b≈16~17(u32 slope 외삽) 필요 → u64 best-DP slope GPU 측정 + 필요시
   고라운드 large 변형. 그리고 최적화(SIMD/병렬)·동결.

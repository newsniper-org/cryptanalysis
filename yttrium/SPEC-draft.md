...
**라운드수 보안레벨(정당화 완료, `milp/yttrium-large-rounds.md`):** digest 256-bit 충돌저항 =
birthday 2¹²⁸. u64 per-round 감쇠가 u32의 **~2배**(16레인; GPU R2=2⁻²¹·⁵, reduced n=16 R1→R2≥+16.6)
→ acc-충돌 w(R_b)≈21.5+~16·(R_b−2): **(10,14,24)-large = full 128-bit 충돌저항**(상한 마진),
(8,12,24)~111-123-bit, (4,6,\*) keyed 전용. **현 변형 충분**. (외삽; floor·width-transfer caveat §해당doc.)
...
6. yhash 대응(u64) yttrium-large: 순열 코어·**전체 Farfalle-tree 모드 구현 완료**(scalar, 가변출력;
   `large.rs`)·**u64 보안레벨 정당화 완료**(`milp/yttrium-large-rounds.md`): u64 per-round 감쇠가
   u32의 **~2배**(16레인; GPU R2=2⁻²¹·⁵, reduced R1→R2≥+16.6) → **(10,14,24)-large가 full 128-bit
   충돌저항**(256-bit digest 상한), (8,12,24)~111-123-bit. **현 변형 충분**(별도 고라운드 불요).
   잔여: 최적화(SIMD/병렬)·동결·deep-slope N↑ 재확인.

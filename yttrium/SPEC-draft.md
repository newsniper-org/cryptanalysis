...
| σ orthomorphism | **all-8 레인** α^{k}, k=[1,2,3,5,7,11,13,17] | §4·§9 — 부분 σ는 약점(MSB-쌍 생존) |
...
- α^k(k∈{1,2,3,5,7,11,13,17})는 전부 **perm + XOR-orthomorphism**(α^k(x)⊕x 치환);
...
```
yᵢ ← α^{kᵢ} · yᵢ    ∀i ∈ {0..7},   k = [1, 2, 3, 5, 7, 11, 13, 17]   (GF(2³²), red 0x400007)
```
...
round(state, r):
    ...
    for i in 0..8: y[i] = α^{k_i}·y[i]   (k=[1,2,3,5,7,11,13,17])   # σ all-8 orthomorphism
...
- **C. 라운드수 정당화** (R2): mixed-algebra bit-level 차분/선형 trail로 R_b·R_c·R_mask 확정.
  현재 4/6/8은 *잠정* (best-DP는 R3서 floor지만 절대 경계 아님).
- **D. mixed-algebra 암호분석**: 영합 가산(⊞, 부호=2의보수)↔GF α^k 상호작용은 단일-대수보다
  난해. rotational-XOR(부호가산이 대칭 깨는지)·boomerang·integral·MSB-쌍 일반화 별도.
- **E. GF primitivity(Q1)·encode 단사(Y1)·mask 단사(Y4)** 이월 가능하나 재확인(Y1·Q1p는 OK).
- **F. 동결 + KAT**(R4): 단일 권위 파라미터(ε·k·(α,β)·π·RC) + 교차구현 KAT.
- **G. constant-time**(R3): 결합기 ROTL/ADD/ROTR + σ α-곱(branchless) — 전부 데이터 독립 →
  ypsilenti와 동급(곱셈 테이블 없음). dudect + 정적 감사.
- **H. 외부 공개 암호분석**(R5): 최종 관문.

## 11. 미해결 / open

1. **라운드수**(§10-C) — 잠정 4/6/8. 절대 trail 경계 필요.
2. **(α,β) 최종값** — (8,9)/(9,10)/(8,3) 접전(차이<2bit, R3 floor 동일). 실익 작음.
3. **σ power 집합 최적화** — all-8 필수는 확정, 단 k=[1,2,3,5,7,11,13,17](59 α-step)은
   비용 큼. 더 작은 distinct 집합이 같은 MSB-쌍 붕괴 + 대각 차단을 주는지 탐색(성능).
4. **ε 패턴** — 교대 [+,−,…] 채택; 다른 영합 패턴의 확산 차이는 미탐(실익 작을 듯).
5. **mixed-algebra 정밀 분석**(§10-D) — 부호가산↔GF α^k rotational/boomerang.
6. yhash 대응(u64) yttrium-large 여부(n=64 prob-1 R\*=2 예비확인됨, `yttrium_lm_subspace.py`).

...
### 2.1 가역성 — *덧셈형 Horst 닫힘(✗), 갈래 미정(§10-A)*
σ-GLM은 `⊕(xᵢ⊕t)=⊕xᵢ`(t 상쇄, n 짝수)로 S 복구가 자명해 가역이었다. ARX 결합기는
이 상쇄가 깨진다. 가역(= 순열) 유지에는 **덧셈형 Horst 조건** `Φ(Σ)=Σ⊞8·F(Σ)` 가
mod 2³² 치환이어야 한다.

> **2³² 전수 검증 결과(완전 증명): Φ는 어떤 c에서도 치환이 아니다**
> (`milp/yttrium-invertibility.md`; 설계값 c=8 이미지 61.48%, c=1 홀수도 51.7%).
> 원인은 `8·F=F≪3`인데 회전 기반 F가 상위 비트로 wrap → T-함수가 아니어서
> `Σ↦Σ⊞F(Σ)`가 비치환. **c·F·σ·(α,β) 어느 튜닝으로도 복구 불가** — 구조적 성질.
> **⟹ 현행 전(全)-레인 ARX broadcast 결합기는 순열이 아니다.**

따라서 §10-A의 갈래는 둘로 좁혀진다(저자 확정 사항):
- **A. 가역 유지:** broadcast를 **일반화 Feistel(Horst)** 로 재구조화(tap-절반→t,
  target-절반만 `⊞=t`, swap). 구성상 가역·F 조건 없음·Farfalle 보존. 비용 ~2배 라운드.
- **B. 비가역 수용:** 현 §2 broadcast 유지(최강 확산), **random-function 압축 모델**
  채택 — 해시 용도엔 가역 불필요, preimage 저항 유리. 단 Farfalle는 순열을 요구하므로
  **모드를 압축함수 트리(Merkle)+Davies–Meyer feed-forward로 교체**(§8/§10-B 연동).
...
- **A. 가역성 확정** (§2.1): ~~덧셈형 Horst로 순열 유지~~ → **2³² 전수로 비치환 확정(✗,
  `milp/yttrium-invertibility.md`)**. 남은 갈래: **A) 일반화 Feistel 재구조화(가역·Farfalle
  보존, ~2배 라운드)** 또는 **B) 비가역 압축 모델(broadcast 유지·모드를 Merkle+DM로 교체)**.
  보안 모델(R1)의 전제이며 §8(B)과 직접 연동. *저자 결정 대기.*
...
1. **가역성**(§2.1, §10-A) — *덧셈형 Horst 전수 비치환 확정(✗)*. 갈래 A(Feistel 재구조화,
   가역) vs B(비가역+Merkle/DM 모드)만 남음. **저자 결정**이 다른 모든 항목(모드·라운드·
   보안모델)의 분기점.

...
- **F. 동결 + KAT**(R4): **KAT 존재**(`yttrium/tests/kat.rs`, 4변형×벡터, *v0.2-pre 미동결*) —
  파라미터(ε·k·(α,β)·π·RC·F·도메인·엔디안) 확정 시 PARAM_VERSION 부여 + 교차구현 대조. *동결 대기.*
- **G. constant-time**(R3): 결합기 ROTL/ADD/SUB/XOR + σ α-곱(branchless `mask=0−(y≫31)`) + F(AND/XOR/회전)
  — **데이터 의존 분기·테이블·메모리접근 없음**(α^k는 *상수* k회 반복). 구성상 CT(곱셈 테이블 없음,
  ypsilenti보다 유리). 레퍼런스는 `#![forbid(unsafe_code)]`. 잔여: dudect 실측 + 정적 감사.

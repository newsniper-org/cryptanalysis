...
| **T2** | 메모리안전 / constant-time | △→✅ `forbid(unsafe_code)`·Zeroize + **정적 CT 감사(data-oblivious) + dudect** (`CT-AUDIT.md`); 형식 CT는 미주장 |
| **T3** | 모드 보안 = 순열을 ideal로 가정한 **환원 증명** | △ Y5 **sorry-free 조건부 정리** + EC 교차검증 (`R1-STATUS.md`); full wiring 잔여 |
| **T4** | 구체 순열의 계산적 보안 = **공개 암호분석** | △ **bit-level 정확 분석**(F-weight·확산 임계값 R*, `milp/bitlevel-analysis.md`); 외부 분석(R5) 잔여 |
...
| # | 항목 | 현재 | 수용 기준 |
|---|------|------|-----------|
| **R4** | **단일 권위 파라미터 동결 + 교차구현 KAT** | ✅ **완료** — `FROZEN-PARAMS.md`(SPEC=코드=Isabelle), `PARAM_VERSION`, `tests/kat.rs`(9벡터/크레이트). drift(yhash 512→1024, ypsi GF `0x1B`→`0x400007`) 정정 | (충족) |
| **R2** | **bit-level 차분/선형 트레일 경계** | △ **부분** — 정확 F-weight(ypsi 2/yhash 4), 확산 임계값 R\*=w(8/16) 확정(`milp/bitlevel-analysis.md`); 채택 라운드 best-trail DP는 미확립(SMT timeout)→GPU/Gurobi | 채택 라운드에서 best trail ≤ 목표(CR 2⁻²⁵⁶/PRF 2⁻¹²⁸) |
| **R1** | **keyed PRF/deck 보안 환원 기계검증** | △ **조건부** — `Y5_CRReduction.thy` sorry-free 조건부 정리(명시적 bound + multi-key) + EC 교차검증(`R1-STATUS.md`); H1/H2 AFP wiring 잔여 | sorry-free + 명시 bound + single→multi-key (무조건) |
| **R3** | **constant-time 검증 (키 경로)** | △ **감사 통과** — 정적 data-oblivious 감사 + dudect 하니스(`CT-AUDIT.md`); 격리환경 clean 측정·asm 검사 잔여 | 정적/dudect + CI 회귀 |
| **R5** | **외부 공개 암호분석** *(최종 관문)* | ❌ 미수행 (GPU 사전증거 모듈은 pre-R5/T4 강화로 별도) | 독립 분석가 공개 리뷰 또는 FSE/ToSC 동료심사 |

→ R1–R5는 frozen on-disk 채택을 검토한 다운스트림 감사에서 도출된 체크리스트로,
충족 시 알고리즘 agility version id로 채택 가능하다. R4 완료, R1–R3 부분 진전,
R5(외부)·R2 정량경계가 잔여 관문. (도구: `PROOF-TOOLCHAIN.md` — CryptHOL 주 + EC 교차검증.)
